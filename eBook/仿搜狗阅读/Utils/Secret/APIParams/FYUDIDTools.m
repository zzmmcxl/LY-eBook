...
static const char kKeyChainUDIDAccessGroup[] = "Q8UZ5842WC.com.fanao";

@implementation FYUDIDTools
+ (NSString*)UDID
{
    NSString *udid = [FYUDIDTools getUDIDFromKeyChain];
    if (!udid) {
        ...
        if (version >= 7.0) {
            udid = [FYUDIDTools _UDID_iOS7];
        }
        else if (version >= 2.0) {
            udid = [FYUDIDTools _UDID_iOS6];
        }
        
        [FYUDIDTools settUDIDToKeyChain:udid];
    }
...
+ (NSString*)_UDID_iOS6
{
    return [FYUDIDTools getMacAddress];
}
/*
 * iOS 7.0
...
+ (BOOL)settUDIDToKeyChain:(NSString*)udid
{
    ...
    if ([FYUDIDTools getUDIDFromKeyChain]) {        // there is item in keychain
        [FYUDIDTools updateUDIDInKeyChain:udid];
        [dictForAdd release];
        return YES;
    }
