...
@interface LSYReadPageViewController ()<UIPageViewControllerDelegate,UIPageViewControllerDataSource,LSYMenuViewDelegate,UIGestureRecognizerDelegate,LSYCatalogViewControllerDelegate,LSYReadViewControllerDelegate>
{
    ...
    NSInteger _startPage;  //因为epud起始页是0 pdf起始页是1
}
...
- (void)initBaseData
{
    _startPage = _model.record.theFirstPage;
...
{
    if (_model.bookType == BookTypePDF) {//pdf暂时不处理
...
-(LSYReadViewController *)readViewWithChapter:(NSUInteger)chapter page:(NSUInteger)page{
    ...
    _readView.bookName = _model.bookName;
    _readView.bookType = _model.bookType;
    _readView.content = [_model.chapters[chapter] stringOfPage:page];
    _readView.delegate = self;
    _readView.currentPage = page;
    NSLog(@"_readGreate");
    
    return _readView;
}

//-(PDFReaderPageVC *)createPDFWithChapter:(NSUInteger)chapter page:(NSUInteger)page{
//    
//    _pageVC = [[PDFReaderPageVC alloc] init];
//    
...
- (nullable UIViewController *)pageViewController:(UIPageViewController *)pageViewController viewControllerBeforeViewController:(UIViewController *)viewController
{
    ...
    if (_chapterChange==0 &&_pageChange <= _startPage) {
        return nil;
    }
    if (_pageChange==_startPage) {
        _chapterChange--;
...
- (nullable UIViewController *)pageViewController:(UIPageViewController *)pageViewController viewControllerAfterViewController:(UIViewController *)viewController
{
    ...
    _pageChange = _page;
    _chapterChange = _chapter;
    if (_pageChange == _model.chapters.lastObject.pageCount-1 && _chapterChange == _model.chapters.count-1) {//最后一章的最后一页 则不翻页
        return nil;
    }
    if (_pageChange == _model.chapters[_chapterChange].pageCount-1) {//本章最后一页 则开始下一章的第一页
        _chapterChange++;
        _pageChange = _startPage;
    }
    else{
        _pageChange++;
    }
