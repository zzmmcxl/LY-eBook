...
#pragma mark - pdf处理
+(NSMutableArray *)pdfFileHandle:(NSURL *)path;
{
...
#pragma mark - 解析PDF文件
+(NSMutableArray *)parsePDF:(NSURL *)pdfPath
{
    LSYChapterModel *model = [[LSYChapterModel alloc]init];
    CGPDFDocumentRef documentRef = CGPDFDocumentCreateUsingUrl((__bridge CFURLRef)pdfPath, @"");
    model.pageCount = CGPDFDocumentGetNumberOfPages(documentRef);
